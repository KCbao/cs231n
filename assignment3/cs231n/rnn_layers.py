...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    hidden_fc = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
    next_h = np.tanh(hidden_fc)

    cache = (Wx, Wh, x, prev_h, hidden_fc)
...
def rnn_step_backward(dnext_h, cache):
    ...
    Wx, Wh, x, prev_h, hidden_fc = cache

    dh_fc = (1 - np.tanh(hidden_fc) ** 2) * dnext_h
    dx = np.dot(dh_fc, Wx.T)
    dprev_h = np.dot(dh_fc, Wh.T)
    dWx = np.dot(x.T, dh_fc)
    dWh = np.dot(prev_h.T, dh_fc)
    db = np.sum(dh_fc, axis=0)
